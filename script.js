// Function to update price text dynamically and store in localStorage
function updatePrice() {
    const priceElement = document.getElementById('priceText');
    const newPrice = (Math.random() * 3 + 2).toFixed(2); // Random price between $2.00 and $5.00
    priceElement.textContent = `Price: $${newPrice} per pound`;

    // Save price to localStorage
    localStorage.setItem('price', newPrice);
}

// Function to modify CSS styles and store the background color in localStorage
function changeDisplay() {
    const productSection = document.querySelector('.product');
    const randomColor = '#' + Math.floor(Math.random() * 16777215).toString(16);
    productSection.style.backgroundColor = randomColor;
    productSection.style.boxShadow = '0 4px 8px rgba(0, 0, 0, 0.2)';
    productSection.style.transition = 'all 0.3s ease';

    // Save background color to localStorage
    localStorage.setItem('backgroundColor', randomColor);
}

// Function to add or remove review section
function toggleReview() {
    const reviewSection = document.getElementById('reviewSection');

    if (reviewSection) {
        // If review exists, remove it
        reviewSection.remove();
    } else {
        // If review doesn't exist, create and add it
        const newReview = document.createElement('article');
        newReview.id = 'reviewSection';
        newReview.className = 'review';
        newReview.innerHTML = '<p>Customer Review: Amazing quality tomatoes, highly recommend!</p>';
        document.querySelector('.product').appendChild(newReview);
    }
}

// Function to load saved preferences from localStorage
function loadPreferences() {
    const savedPrice = localStorage.getItem('price');
    const savedColor = localStorage.getItem('backgroundColor');

    if (savedPrice) {
        document.getElementById('priceText').textContent = `Price: $${savedPrice} per pound`;
    }

    if (savedColor) {
        document.querySelector('.product').style.backgroundColor = savedColor;
    }
}

// Load preferences on page load
window.onload = loadPreferences;

// Event listeners for buttons
document.getElementById('changePriceBtn').addEventListener('click', updatePrice);
document.getElementById('changeColorBtn').addEventListener('click', changeDisplay);
document.getElementById('toggleReviewBtn').addEventListener('click', toggleReview);
